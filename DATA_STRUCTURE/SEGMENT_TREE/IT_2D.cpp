...
struct segmentTree {
	...
	void update(const node &dx, const node &dy,
				int u,int v,type val, bool all){
		if (dx.irrelevant(u,u) || dy.irrelevant(v,v))
			return;
	...
	type query(const node &dx, const node &dy,
				int u,int v,int uu,int vv){
		if (dx.irrelevant(u,uu) || dy.irrelevant(v,vv))
			return 0;
		if (u<=dx.ll && dx.rr<=uu){
			if (v<=dy.ll && dy.rr<=vv)
				return T[dx.id][dy.id];
	...
	while (test--){
		...
		while (cin>>s && s!="END"){
			if (s=="SET"){
				cin>>u>>v>>val;
				u++; v++;
				valold=it.query(dx,dy,u,v,u,v);
				it.update(dx,dy,u,v,val-valold,true);
			} else if (s=="SUM"){
				cin>>u>>v>>uu>>vv;
				u++; v++; uu++; vv++;
				cout<<it.query(dx,dy,u,v,uu,vv)<<"\n";
			}
		}
	}
}
